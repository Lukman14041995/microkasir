                ...
                <div class="card">
					...
					?>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    <h4 class="mt-5">Penjualan Hari Ini, <?php echo date('j F Y, G:i') ?> <a href="report.php" class="btn btn-primary" style="margin-left: 30px;">Details</a></h4>
                                    <div class="col-md-6">
                                    <p>Original</p>
                                        <?php 
                                        require "konfig.php";
                                        ...
                                        while ($a = mysqli_fetch_array($sql)) {
                                            $jumlah += $a['jumlah'];
                                        ?>
                                        <p style="margin-top: -10px;"><?php echo $a['nama_barang']?> : <?php echo $a['jumlah'] ?></p>
                                        <?php } 
                                        $date =  date('Y-m-d');
                                        $q = mysqli_query($koneksi, "SELECT SUM(jumlah) as jumlah FROM nota WHERE periode like '%". $date ."%'");
                                        $data = mysqli_fetch_array($q);
                                        ?>
                                        <hr style="margin-top: -10px;">
                                        <p style="margin-top: -10px;">Total : <?php echo $data['jumlah'] ?></p>
                                    </div>
                                    <div class="col-md-6">
                                    <p>UMKM </p> 
                                        <?php 
...
$q = mysqli_query($koneksi, "SELECT transaksi.bulan, SUM(transaksi.totalsemua) AS totalsemua, bulan.nama_bulan, bulan.id
FROM transaksi 
INNER JOIN bulan ON transaksi.bulan = bulan.nama_bulan 
WHERE transaksi.id_cabang = '$id' 
GROUP BY transaksi.bulan ORDER BY bulan.id asc");
